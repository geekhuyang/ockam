defmodule Ockam.Protocol.Stream.BiDirectional do
  ...
  def protocol() do
    %Ockam.Protocol{
      ...
      request: {:struct, [stream_name: :string, subscription_id: {:optional, :string}]}
    }
  end
end
...
defmodule Ockam.Stream.Client.BiDirectional.Subscribe do
  ...
  @protocol_mapping Ockam.Protocol.Mapping.mapping([{:server, SubscribeProtocol}, {:server, PublisherProtocol}])
  ...
  def handle_message(message, state) do
    case decode_payload(Message.payload(message)) do
      {:ok, SubscribeProtocol, %{stream_name: stream_name, subscription_id: subscription_id}} ->
        subscription_id = case subscription_id do
          :undefined -> "default"
          _subscription_id -> subscription_id
        end
        BiDirectional.subscribe(stream_name, subscription_id, Map.fetch!(state, :stream_options))
      {:ok, PublisherProtocol, %{publisher_stream: publisher_stream, consumer_stream: consumer_stream, subscription_id: subscription_id}} ->
        {:ok, address} = BiDirectional.ensure_publisher(consumer_stream, publisher_stream, subscription_id, Map.fetch!(state, :stream_options))
        Ockam.Router.route(%{
